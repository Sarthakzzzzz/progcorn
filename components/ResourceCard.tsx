...
export default function ResourceCard({ resource }: { resource: Resource }) {
  ...
  const upvote = async () => {
    ...
    setLoading(true)
    try {
      await api(`/resources/${resource.id}/upvote`, { method: 'POST', auth: true })
      setCount((c) => c + 1)
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 flex gap-4">
      <div className="flex flex-col items-center">
        <button onClick={upvote} disabled={loading} className="rounded-md border border-slate-300 bg-white px-2 py-1 text-xs hover:bg-slate-50">▲</button>
